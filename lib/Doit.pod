...
An experimental extension which allows changed the file mode using
symbolic or ls modes can be found here:
L<https://github.com/eserte/Doit-Experiments/blob/master/lib/DoitX/Chmod.pm>.
...
possibly replacing an existing symlink. Implemented using Perl's
built-in C<symlink> and C<rename> functions, which is equivalent to
running C<ln -nsf> but does not depend on the system's C<ln> supporting
the C<-n> option. If the environment variable
C<DOIT__USE_LN_NSF_SYSTEM> is set to a true value, then the system's
C<ln -nsf> command is used instead. See L</symlink> for an alternative
that does not replace an existing symlink.
...
If the option C<< show_diff=>1 >> is specified, then a diff output is
shown in the case the destination file already exists.
...
=head3 rename
    ...
    $doit->rename($from, $to);
    $doit->rename({show_diff=>1}, $from, $to);
...
move a file between different filesystems. If in doubt, prefer to use
C<move>.
...
If C<quiet> is set to a true value, then no logging is done. If
C<info> is set to a true value, then command execution happens even in
dry-run mode.
...
=head3 info_run
    ...
    $doit->info_run(...);
    $doit->info_run({quiet => $bool}, ...);

Like L</run>, but with the option C<< info=>1 >> set.
...
=item C<DOIT__USE_LN_NSF_SYSTEM>

If set to a true value, the L</ln_nsf> command uses the system's
C<ln -nsf> command instead of the default pure-Perl implementation.
