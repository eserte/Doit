...
=item C<< { match => $rx, replace => $line } >>

Substitute all lines matching I<$rx> with I<$line>. Instead of an
regexp a string may be specified; in this case the complete line has
to match.

=item C<< { match => $rx, action => $code } >>

For all lines matching I<$rx> call the specified I<$code> reference.
In the code reference I<$_[0]> can be used to manipulate the line.
...
This is probably not very useful (just think of running a non-matching
